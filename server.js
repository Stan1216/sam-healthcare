...
function sharenRules(t) {
  ...
  if (/(chest pain|unconscious|not breathing|breathless|severe bleeding|stroke|seizure|suicide|overdose|heart attack|electric shock|drown|choking)/.test(t))
    return A("This sounds like it could be an emergency. Please call 108 (ambulance) right now — do not wait for a home visit or a chat reply. Once emergency care is underway, our care team can help coordinate hospital handoff and follow-up at home.", ["Emergency routing per SAM safety charter — Dr. Sharen never gates emergencies"], "emergency", true);
...
HARD RULES: Never diagnose. Never name a specific medicine or dose. Emergencies (chest pain, breathlessness, stroke signs, heavy bleeding, unconsciousness) -> tell them to call 108 NOW. Be warm, empathetic, never boastful. Max ~120 words.
...
http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/api/agent/reply" && req.method === "POST") {
    ...
    if (handoff) { reply = "You're with our human care team now — they have your full conversation and will reply here shortly. 🙏"; }
      ...
      reply = "⚠️ This could be an emergency. Please call 108 (ambulance) RIGHT NOW — don't wait for a message reply. Our care team is being alerted and can help coordinate hospital handoff and follow-up at home."; handoff = true;
    }
